...
// Feriados nacionais fixos + móveis estimados para anos próximos
function getFeriadosNacionais(ano: number): { date: Date; nome: string }[] {
  const fixos = [
    { m: 0, d: 1, nome: "Confraternização Universal" },
    { m: 3, d: 21, nome: "Tiradentes" },
    { m: 4, d: 1, nome: "Dia do Trabalhador" },
    { m: 8, d: 7, nome: "Independência do Brasil" },
    { m: 9, d: 12, nome: "Nossa Sra. Aparecida" },
    { m: 10, d: 2, nome: "Finados" },
    { m: 10, d: 15, nome: "Proclamação da República" },
    { m: 10, d: 20, nome: "Dia da Consciência Negra" },
  ...
  moveis.forEach(mv => {
    ...
    d.setDate(d.getDate() + mv.offset);
    feriados.push({ date: d, nome: mv.nome });
  });
...
function isFeriado(date: Date, feriados: { date: Date; nome: string }[]): { is: boolean; nome?: string } {
  for (const f of feriados) {
    if (f.date.getFullYear() === date.getFullYear() &&
...
interface Resultado {
  ...
  feriadosNoPeriodo: { date: Date; nome: string }[];
}
...
export function PrazoCalc() {
  ...
  const calcular = () => {
    ...
    // CPC art. 224 §1º: exclui-se o dia do começo, inclui-se o do vencimento
    let date = new Date(dataIntimacao);
    date.setDate(date.getDate() + 1);

    // Se primeiro dia não é útil, começa no próximo dia útil (art. 224 §2º)
    if (diasUteis) {
      while (!isDiaUtil(date, feriados)) {
        date.setDate(date.getDate() + 1);
      }
    }
    ...
    if (diasUteis) {
      let counted = 0;
      while (counted < diasPrazo) {
        if (isDiaUtil(date, feriados)) {
          counted++;
        }
        if (counted < diasPrazo) date.setDate(date.getDate() + 1);
      }
    } else {
      date.setDate(date.getDate() + diasPrazo - 1);
    }

    // Se vencimento cai em dia não útil, prorroga (art. 224 §1º)
    if (diasUteis) {
      while (!isDiaUtil(date, feriados)) {
        date.setDate(date.getDate() + 1);
      }
    ...
    // Feriados que caem no período
    const feriadosNoPeriodo = feriados.filter(f =>
      f.date >= inicioContagem && f.date <= date &&
      f.date.getDay() !== 0 && f.date.getDay() !== 6
    );

    const diasRestantes = differenceInCalendarDays(date, new Date());

    setResult({ dataVencimento: date, diasRestantes, feriadosNoPeriodo });
  };
  ...
  const baixarICS = () => {
    ...
    a.href = url;
    a.download = `prazo-${format(result.dataVencimento, "yyyy-MM-dd")}.ics`;
    a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className={cn("w-full justify-start text-left font-normal", !dataIntimacao && "text-muted-foreground")}
              >
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <span className="text-sm text-muted-foreground">{diasUteis ? "Sim — dias úteis" : "Não — dias corridos"}</span>
          </div>
        </div>
      </div>

      <Button onClick={calcular} className="w-full sm:w-auto" disabled={!dataIntimacao || !diasPrazo}>
        Calcular Prazo
      </Button>
      ...
      {result && (
        <div className="space-y-4">
          ...
          <Card className={bgPrazo}>
            <CardContent className="pt-6 space-y-3">
              ...
              <div className={cn("text-sm font-medium", corPrazo)}>
                {result.diasRestantes < 0
                  ? `⚠️ Prazo vencido há ${Math.abs(result.diasRestantes)} dia(s)!`
                  : result.diasRestantes === 0
                    ? "⚠️ O prazo vence HOJE!"
                    : `⏳ Faltam ${result.diasRestantes} dia(s) corridos para o vencimento`}
              </div>
          ...
          {/* Informações sobre CPC art. 219 */}
          <Card className="border-muted">
            <CardContent className="pt-6 space-y-2 text-sm text-muted-foreground">
              <p className="font-semibold text-foreground">📖 Base Legal — CPC Art. 219 e 224</p>
              <p>
                <strong>Art. 219:</strong> Na contagem de prazo em dias, estabelecido por lei ou pelo juiz, 
                computar-se-ão somente os dias úteis.
              </p>
              <p>
                <strong>Art. 224 §1º:</strong> Exclui-se o dia do começo e inclui-se o do vencimento do prazo.
              </p>
              <p>
                <strong>Art. 224 §2º:</strong> Os prazos fixados em dias úteis consideram-se 
                prorrogados até o primeiro dia útil seguinte, quando o vencimento cair em dia 
          ...
          <div className="flex items-start gap-2 rounded-lg border border-yellow-500/30 bg-yellow-50 dark:bg-yellow-950/20 p-4">
            ...
            <p className="text-sm text-yellow-700 dark:text-yellow-300">
              Sempre confirme os prazos no sistema do tribunal. Feriados locais, pontos facultativos e 
              suspensões de prazo podem alterar o resultado. Este cálculo considera apenas feriados nacionais.
            </p>
          </div>
        </div>
      )}
