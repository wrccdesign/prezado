...
export function RescisaoCalc() {
  ...
  const calcular = () => {
    ...
    if (!sal || !admissao || !demissao) {
      toast.error("Preencha salário, data de admissão e demissão.");
      return;
    }
    if (demissao <= admissao) {
      toast.error("A data de demissão deve ser posterior à admissão.");
      return;
    }
    ...
    if (tipo === "sem_justa_causa") {
      ...
      avisoPrevio = (remuneracao / 30) * diasAviso;
    } else if (tipo === "acordo_mutuo") {
      const diasAviso = Math.min(30 + anosCompletos * 3, 90);
      avisoPrevio = ((remuneracao / 30) * diasAviso) / 2;
    }
    ...
    if (tipo !== "com_justa_causa") {
      decimoTerceiro = (remuneracao / 12) * meses13;
    }
    ...
    if (tipo !== "com_justa_causa") {
      feriasProporcionais = (remuneracao / 12) * mesesFerias;
    ...
    if (periodosCompletos >= 1) {
      feriasVencidas = remuneracao;
      tercoFeriasVencidas = remuneracao / 3;
    }
    ...
    let multaFgts = 0;
    if (tipo === "sem_justa_causa") {
      multaFgts = (fgts + fgtsMes) * 0.4;
    } else if (tipo === "acordo_mutuo") {
      multaFgts = (fgts + fgtsMes) * 0.2;
    }
  ...
  const gerarPDF = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Relatório de Rescisão Trabalhista — JurisAI", 14, 20);
    doc.setFontSize(10);
    doc.text(`Data: ${format(new Date(), "dd/MM/yyyy")}`, 14, 28);
    ...
    doc.setFontSize(10);
    verbas.forEach((v) => {
      doc.text(v.label, 14, y);
      doc.text(fmt(v.valor), 140, y);
      y += 7;
    });
    ...
    doc.text(fmt(total), 140, y);

    y += 14;
    doc.setFontSize(8);
    doc.text("Cálculo estimado. Consulte um advogado trabalhista para valores exatos.", 14, y);

    doc.save("rescisao-trabalhista-jurisai.pdf");
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !admissao && "text-muted-foreground")}>
                <CalendarIcon className="mr-2 h-4 w-4" />
                {admissao ? format(admissao, "dd/MM/yyyy") : "Selecione a data"}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={admissao} onSelect={setAdmissao} initialFocus className="p-3 pointer-events-auto" locale={ptBR} />
            </PopoverContent>
          </Popover>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !demissao && "text-muted-foreground")}>
                <CalendarIcon className="mr-2 h-4 w-4" />
                {demissao ? format(demissao, "dd/MM/yyyy") : "Selecione a data"}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={demissao} onSelect={setDemissao} initialFocus className="p-3 pointer-events-auto" locale={ptBR} />
            </PopoverContent>
          </Popover>
        </div>

        <div className="space-y-2">
          <Label>Média de Horas Extras Mensais (R$)</Label>
          <Input type="number" placeholder="0" value={horasExtras} onChange={e => setHorasExtras(e.target.value)} />
        </div>

        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <Switch checked={possuiComissoes} onCheckedChange={setPossuiComissoes} />
            <Label>Possui comissões?</Label>
          </div>
          {possuiComissoes && (
            <div className="space-y-2">
              <Label>Média Mensal de Comissões (R$)</Label>
              <Input type="number" placeholder="0" value={comissoes} onChange={e => setComissoes(e.target.value)} />
            </div>
          )}
        </div>
        ...
        <div className="space-y-4 sm:col-span-2">
          <div className="flex items-center gap-3">
            <Switch checked={possuiFgts} onCheckedChange={setPossuiFgts} />
            <Label>Possui saldo de FGTS?</Label>
          </div>
          {possuiFgts && (
            <div className="space-y-2 max-w-xs">
              <Label>Saldo Estimado do FGTS (R$)</Label>
      ...
      {verbas && (
        <div className="space-y-4">
                ...
                <TableBody>
                  {verbas.map((v) => (
                    <TableRow key={v.label}>
                      ...
                      <TableCell className="text-right font-medium">{fmt(v.valor)}</TableCell>
                    </TableRow>
                  ))}
                  <TableRow className="border-t-2">
                    <TableCell className="text-base font-bold">Total Líquido Estimado</TableCell>
                    <TableCell className="text-right text-base font-bold text-primary">{fmt(total)}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </CardContent>
          ...
          <div className="flex items-start gap-2 rounded-md border border-yellow-500/30 bg-yellow-50 dark:bg-yellow-950/20 p-3 text-sm text-yellow-800 dark:text-yellow-200">
            ...
            <span>Cálculo estimado. Consulte um advogado trabalhista para valores exatos.</span>
          </div>
        </div>
      )}
    </div>
