...
function classifyLine(line: string, index: number, totalLines: number, prevTypes: LineType[]): LineType {
  ...
  // Header: first ~5 lines with keywords
  if (index < 8 && HEADER_KEYWORDS.some(k => upper.includes(k))) {
    return "header";
  }

  // Section titles: all caps patterns like "DOS FATOS", "DO DIREITO", "DOS PEDIDOS"
  if (SECTION_TITLE_REGEX.test(trimmed) && trimmed === trimmed.toUpperCase() && trimmed.length < 80) {
    return "section-title";
  }
  // Also match standalone section headers
  const sectionPatterns = [
    /^(DOS?\s+FATOS|DA\s+FUNDAMENTA|DO\s+DIREITO|DOS?\s+PEDIDOS|DA\s+TUTELA|DO\s+VALOR|DA\s+CAUSA|DAS?\s+PROVAS|DA\s+COMPET|PRELIMINAR)/i
  ];
  if (sectionPatterns.some(p => p.test(trimmed)) && trimmed.length < 80) {
    return "section-title";
  }

  // Party identification
  if (PARTY_KEYWORDS.some(k => upper.includes(k)) && index < 20) {
    return "party";
  }

  // Closing: near the end with keywords
  if (index > totalLines - 15 && CLOSING_KEYWORDS.some(k => upper.includes(k))) {
    return "closing";
  }
  // Lines after closing keywords are also closing
  if (index > totalLines - 12 && prevTypes.length > 0 && prevTypes[prevTypes.length - 1] === "closing") {
    return "closing";
  }
...
export function PetitionResult({ text, petitionType, onNewPetition }: PetitionResultProps) {
  ...
  const handleDownloadPDF = () => {
    ...
    const bodyIndent = 12.5; // 1.25cm first line indent
    const dateStr = getFormattedDate();
    const warningText = "Gerado por JurisAI — Documento deve ser revisado por advogado habilitado";

    const addPageHeaderFooter = (d: jsPDF) => {
      d.setFont("times", "bold");
      d.setFontSize(10);
      d.text("JurisAI", mLeft, 15);
      d.setFont("times", "normal");
      d.setFontSize(9);
      d.text(dateStr, pageWidth - mRight, 15, { align: "right" });
      d.setDrawColor(150);
      d.line(mLeft, 18, pageWidth - mRight, 18);
    ...
    const checkPage = (needed: number) => {
      if (y + needed > pageHeight - mBottom - 12) {
        doc.addPage();
        addPageHeaderFooter(doc);
        y = mTop;
      }
    ...
    for (const line of parsed) {
      ...
      if (line.type === "closing") {
        ...
        doc.setFontSize(12);
        checkPage(6);
        doc.text(trimmed, pageWidth - mRight, y, { align: "right" });
        y += 6;
        continue;
      }

      // body - justified with first line indent
      doc.setFont("times", "normal");
      doc.setFontSize(12);
      ...
      for (let wi = 0; wi < wrapped.length; wi++) {
        ...
        // Simple justified text via splitTextToSize
        doc.text(wrapped[wi], xPos, y, {
          maxWidth: lineWidth,
          align: wi === 0 ? "left" : "justify",
        });
        y += 6;
      }
    }
    ...
    doc.setFont("times", "normal");
    doc.setFontSize(12);
    doc.text("___________________________________________", pageWidth / 2, y, { align: "center" });
    y += 6;
  ...
  const handleDownloadDocx = async () => {
    ...
    for (const line of parsed) {
      ...
      if (line.type === "header") {
        docParagraphs.push(new Paragraph({
          children: [new TextRun({ text: trimmed, font: "Times New Roman", size: 26, bold: true })],
      ...
      if (line.type === "section-title") {
        docParagraphs.push(new Paragraph({
          children: [new TextRun({ text: trimmed.toUpperCase(), font: "Times New Roman", size: 24, bold: true })],
          alignment: AlignmentType.CENTER,
      ...
      if (line.type === "party") {
        docParagraphs.push(new Paragraph({
          children: [new TextRun({ text: trimmed, font: "Times New Roman", size: 24, bold: true })],
    ...
    docParagraphs.push(new Paragraph({
      children: [new TextRun({ text: "___________________________________________", font: "Times New Roman", size: 24 })],
      alignment: AlignmentType.CENTER,
    }));
    docParagraphs.push(new Paragraph({
      children: [new TextRun({ text: "Advogado(a) / OAB", font: "Times New Roman", size: 24 })],
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 },
    }));
    ...
    const docFile = new Document({
      sections: [{
        properties: {
          page: {
            margin: { top: 1701, left: 1701, bottom: 1134, right: 1134 },
          },
        },
        headers: {
          default: new Header({
          ...
          default: new Footer({
            children: [new Paragraph({
              children: [new TextRun({
                text: "Gerado por JurisAI — Documento deve ser revisado por advogado habilitado",
