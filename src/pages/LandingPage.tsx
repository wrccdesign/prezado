...
import {
  ...
  ArrowRight, Check, Star, Scale, ChevronRight, Menu, X, MapPin, Database, Users
} from "lucide-react";
...
const coverageSources = [
  { icon: Database, title: "CNJ / DataJud", desc: "Todos os tribunais são obrigados por lei a reportar ao CNJ. Cobertura garantida e atualizada diariamente." },
  { icon: MapPin, title: "Portais dos tribunais", desc: "Scraping direto nos sistemas e-SAJ, PJe e portais próprios para capturar ementas e texto integral das decisões." },
  { icon: Users, title: "Crowdsourcing verificado", desc: "Advogados contribuem com decisões raras do interior. Cada upload é validado pela IA antes de publicar." },
];
...
export default function LandingPage() {
  ...
  return (
    <div ref={revealRef} className="min-h-screen font-sans">
            ...
            <div data-reveal className="opacity-0 translate-y-6 transition-all duration-700">
              ...
              <div className="flex flex-wrap gap-3 mb-10">
                <Button size="lg" className="bg-gold text-navy hover:bg-gold-light font-semibold text-base px-8" asChild>
                  <Link to="/auth">Começar Grátis <ArrowRight className="ml-2 h-5 w-5" /></Link>
                </Button>
              ...
              <div className="flex gap-8 text-sm">
                {[
                  { value: "27", label: "Tribunais cobertos" },
                  { value: "5k+", label: "Decisões indexadas" },
                  { value: "100%", label: "Dados públicos oficiais" }
                ].map((s) => (
                  <div key={s.label}>
            ...
            <div data-reveal className="opacity-0 translate-y-6 transition-all duration-700 delay-200 hidden lg:block">
              <div className="animate-float rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 max-w-md ml-auto shadow-2xl">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <div className="text-white font-semibold text-sm">Prezado.ai</div>
                ...
                <div className="space-y-3">
                  {[
                    { icon: Search, label: "Busca de Jurisprudência", status: "Ativo" },
                    { icon: Stethoscope, label: "Diagnóstico Jurídico", status: "Ativo" },
                    { icon: FileText, label: "Geração de Petições", status: "Ativo" },
      ...
      {/* ========== DIFERENCIAIS BAND ========== */}
      <section style={{ backgroundColor: "hsl(var(--navy-medium))" }} className="py-8 border-b" style={{ borderBottomColor: "hsl(var(--gold) / 0.1)" }}>
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div data-reveal className="opacity-0 translate-y-6 transition-all duration-700 text-center mb-14">
            ...
            <h2 className="font-serif text-3xl md:text-4xl font-bold text-white mt-3">
              Jurisprudência do Brasil inteiro —{" "}
              <em className="text-gold not-italic">inclusive do interior</em>
            </h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {coverageSources.map((s, i) => (
              <div key={s.title} data-reveal className="opacity-0 translate-y-6 transition-all duration-500 rounded-xl border border-white/10 bg-white/5 p-6" style={{ transitionDelay: `${i * 100}ms` }}>
                <div className="h-11 w-11 rounded-xl bg-gold/10 flex items-center justify-center mb-4">
                  <s.icon className="h-5 w-5 text-gold" />
                </div>
                <h3 className="font-serif font-bold text-white text-lg mb-2">{s.title}</h3>
                <p className="text-sm text-white/50 leading-relaxed">{s.desc}</p>
              </div>
            ))}
          </div>

          <div data-reveal className="opacity-0 translate-y-6 transition-all duration-700 text-center mt-10">
        ...
        <div className="container mx-auto px-4 sm:px-6">
          ...
          <div className="border-t pt-5 flex flex-col items-center gap-2 text-center" style={{ borderColor: "hsl(var(--gold) / 0.1)" }}>
            ...
            <p className="text-xs text-white/25">
              Desenvolvido por{" "}
              <a href="https://www.wrcc.design" target="_blank" rel="noopener noreferrer" className="hover:text-gold transition-colors underline underline-offset-2">
                WRCC Design
              </a>
            </p>
          </div>
