...
export default function Chat() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <SEO title="Chat Jurídico com IA — HonorÃ­fico" description="Converse com a IA jurídica do HonorÃ­fico sobre qualquer tema do Direito brasileiro, com respostas fundamentadas." path="/chat" />
      <LegalDisclaimer />
      <main className="flex flex-1 flex-col container max-w-3xl py-4">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto pb-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-primary">
                <img src={logoImg} alt="HonorÃ­fico" className="h-10 w-10 object-contain" />
              </div>
              <h3 className="text-lg font-semibold text-foreground">HonorÃ­fico Chat</h3>
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              ...
              <Card className={`max-w-[85%] px-4 py-3 ${msg.role === "user" ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
                {msg.role === "assistant" ? (
                  <div className="prose prose-sm max-w-none dark:prose-invert">
                    <ReactMarkdown components={{
                      a: ({ href, children }) => (
                        <a href={href} target="_blank" rel="noopener noreferrer" className="text-primary underline decoration-primary/30 hover:decoration-primary font-medium">{children}</a>
                      ),
        ...
        <div className="sticky bottom-0 border-t bg-background pt-4">
          <div className="flex gap-2">
            <Textarea placeholder="Digite sua dúvida jurídica..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} disabled={isLoading} className="min-h-[48px] max-h-[120px] resize-none" rows={1} />
            <Button size="icon" onClick={handleSend} disabled={isLoading || !input.trim()} className="h-12 w-12 shrink-0">
