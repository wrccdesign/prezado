  ...
  try {
    ...
    if (authHeader) {
      ...
      if (user) {
        ...
        const { allowed, used, limit, plan, renewsAt } = await checkRateLimit(user.id, "search", SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, env);
        if (!allowed) {
          return new Response(JSON.stringify({
            error: monthlyLimitMessage("search", limit, plan),
