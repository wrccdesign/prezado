DROP FUNCTION IF EXISTS public.search_decisions_vector(extensions.vector, double precision, integer);

CREATE FUNCTION public.search_decisions_vector(
  query_embedding extensions.vector,
  match_threshold double precision DEFAULT 0.3,
  match_count integer DEFAULT 10,
  only_julgados boolean DEFAULT false
)
RETURNS TABLE(
  ...
  score_utilidade integer, source_url text, created_at timestamp with time zone,
...
AS $function$
  ...
  WHERE d.embedding IS NOT NULL
    ...
    AND (only_julgados IS NOT TRUE OR d.resultado IS NOT NULL)
  ORDER BY d.embedding OPERATOR(extensions.<=>) query_embedding
  LIMIT match_count;
$function$;

GRANT EXECUTE ON FUNCTION public.search_decisions_vector(extensions.vector, double precision, integer, boolean) TO anon, authenticated, service_role;

DROP FUNCTION IF EXISTS public.search_decisions(text, text, text, text, text, boolean, integer, integer);

CREATE FUNCTION public.search_decisions(
  search_query text,
  filter_tribunal text DEFAULT NULL::text,
  filter_uf text DEFAULT NULL::text,
  filter_ramo text DEFAULT NULL::text,
  filter_instancia text DEFAULT NULL::text,
  filter_comarca_pequena boolean DEFAULT NULL::boolean,
  result_limit integer DEFAULT 20,
  result_offset integer DEFAULT 0,
  filter_only_julgados boolean DEFAULT NULL::boolean
)
RETURNS TABLE(
  id uuid, tribunal text, instancia text, uf text, comarca text,
  numero_processo text, data_decisao date, relator text, orgao_julgador text,
...
AS $function$
  ...
  WHERE
    ...
    AND (filter_only_julgados IS NOT TRUE OR d.resultado IS NOT NULL)
  ORDER BY rank DESC
  LIMIT result_limit
  OFFSET result_offset;
$function$;

GRANT EXECUTE ON FUNCTION public.search_decisions(text, text, text, text, text, boolean, integer, integer, boolean) TO anon, authenticated, service_role;